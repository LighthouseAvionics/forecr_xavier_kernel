 ...
 *
 * nv_imx412.c - imx412 sensor driver
 */
...
static int imx412_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	frame_length = (u32) (IMX412_SENSOR_INTERNAL_CLK_FREQ *
...
static int imx412_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	fine_integ_time_factor = priv->fine_integ_time *
		...
		IMX412_SENSOR_INTERNAL_CLK_FREQ;
...
static int imx412_set_mode(struct tegracam_device *tc_dev)
{
	...
	uint offset = ARRAY_SIZE(imx412_frmfmt);
	...
	err = imx412_write_table(priv, mode_table[IMX412_MODE_COMMON]);
	if (err)
		return err;

	mode_index = s_data->mode;
	if (priv->config == FOUR_LANE_CONFIG)
		err = imx412_write_table(priv, mode_table[mode_index + offset]);
	else
		err = imx412_write_table(priv, mode_table[mode_index]);
...
#endif
{
	...
	priv = (struct imx412 *)s_data->priv;
...
MODULE_DESCRIPTION("Media Controller driver for Sony IMX412");
